...
uses
  ...
  StdCtrls, Menus, ExtCtrls, ActnList, Spin, Buttons, ButtonPanel, uib,
  fpspreadsheetgrid, fpsallformats,LSystemTrita, umEdit;
...
begin
  db.Connected:=True;
  riga:= 2;
  st1 := 'insert into codicivestiario (descrizione,codice,rimanenza) values (';
  Qtemp.SQL.Text:= st;
  for riga:= 2 to WorksheetGrid.RowCount -1 do
   begin
       st:= st1;
       if WorksheetGrid.cells[1,riga] = null then
         tmp:= ' '
       else
         tmp :=  WorksheetGrid.cells[1,riga];
       st:= st + AddValIntoSql(tmp,[AsqlVirgola,AsqlApici,AsqlUpper]);

       if WorksheetGrid.cells[2,riga] = null then
         tmp:= ' '
       else
         tmp :=  WorksheetGrid.cells[2,riga];
       st:= st + AddValIntoSql(tmp,[AsqlVirgola,AsqlApici,AsqlUpper]);

       if WorksheetGrid.cells[3,riga] = null then
         tmp:= '0'
       else
         tmp :=  WorksheetGrid.cells[3,riga];

       st:= st + tmp + ')';

       Memo1.Lines.Add(inttostr(riga) + ' ' + st );
       Qtemp.SQL.Text:=st;
       Qtemp.Open;
end;
tr.Commit;
